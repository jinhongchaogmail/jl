...
function train_model(train_x, train_y, val_x, val_y, model, loss, opt, train_losses, val_losses, epochs)

    # 将数据转换为适合 DataLoader 的格式
    train_data = [(train_x[i, :, :], train_y[i]) for i in 1:size(train_x, 1)]
    val_data = [(val_x[i, :, :], val_y[i]) for i in 1:size(val_x, 1)]

    # 使用 DataLoader 处理数据，并开启异步加载
    train_data = Flux.Data.DataLoader(train_data, batchsize=32, async=true)
    val_data = Flux.Data.DataLoader(val_data, batchsize=32, async=true)
...
for window_size in 6:30
    ...
    # 训练模型
    train_model(train_x0, train_y0, val_x0, val_y0, model, loss, opt, train_losses, val_losses, epochs)
